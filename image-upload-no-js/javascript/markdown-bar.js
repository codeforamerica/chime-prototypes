...
function MarkdownBar(bar, textarea) {
	...
	//Define Markdown Patterns in order of how they should appear in the markdown bar
	var PATTERNS = [
		{
			'name': 'Bold',
			'syntax': '**${content}**',
		...
		{
			'name': 'Image',
			'syntax': '![${image}](${caption})',
			'icon': '<span class="fa fa-picture-o"></span>',
			'filler': "This is an image",
			'type': 'inline'
	...
	// Call out custom patterns that need special treatment.
	var CUSTOM_PATTERNS = [
		"Image"
	];
	...
	this.init = function() {
		...
		$(PATTERNS).each(function(index, pattern) {
			var className = pattern['name'].replace(/\s/g, '-').toLowerCase();
			var patternButton = $('<div class="button toolbar__item markdown-bar__button markdown-bar__button--' + className + '">' + pattern['icon'] + '<div class="markdown-bar__tooltip"><span class="markdown-bar__tooltip-title">' + pattern['name'] + '</span></div></div>');

			if(CUSTOM_PATTERNS.indexOf(pattern["name"]) < 0) {
		...
		// Add Image Button Behavior
		$('.markdown-bar__button--image').find('.markdown-bar__tooltip').append('<form class="image-uploader"><label for="image-uploader__input" class="button">upload an image</label><input type="file" id="image-uploader__input"/><input type="text" placeholder="caption/alt text"><button type="submit" class="button button--blue">Add</button></form>');
		$('.markdown-bar__button--image').click(function(e) {
			if($(e.target).hasClass('markdown-bar__button--image') || $(e.target).hasClass('fa-picture-o')) {
				event.preventDefault();
